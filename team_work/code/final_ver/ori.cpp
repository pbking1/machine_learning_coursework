...
void readvalues(){
	...
	int temp = -2;
	char *ptrnext = NULL;
	char *tokenptr = strtok_s(c, ",", &ptrnext);
	while (tokenptr != NULL){
		if (temp >= 0)
			features[temp] = tokenptr;
		tokenptr = strtok_s(NULL, ",", &ptrnext);
		temp++;
	}
	for (int i = 0; i<ITEM_SIZE; i++){
		...
		char *token = strtok_s(c, ",", &temp_ptrnext);
		while (token != NULL){
			if (temp == -2)
				temp = -2;
			else if (temp == -1)
			...
			token = strtok_s(NULL, ",", &temp_ptrnext);
			temp++;
		}
	}
}
...
Node* create_node(vector<int> chosen_ids){
	...
	if (tree_node > NODE_NUMBER || all_items_size < MIN_NODE_NUMBER){
		...
		fout.open(fname);
		fout << "\n\n";
		cout << "this is the: " << newnode->node_id << "th node of the DT." << endl;
...
void compute_test(Node *root){
	...
	for (int item = 1; item <= TEST_ITEM_SIZE; item++){
		...
		while (Root->label == -1){
			...
			cout << node_feature << endl;
			cout << position << endl;
			vector<double> item_value = test_id_values[item];
			cout << item_value.at(0) << " " << item_value.at(1) << endl;
...
int main(){
	...
	read_test_values();
	//-----------------------------------------choose items-------------------------------------
	cout << "Start randomly choosing ids and features" << endl;
	...
	//-------------------------------------------compute test data-------------------------------
	cout << "Start computing test data" << endl;
	compute_test(Root);
	for (int i = 0; i < TEST_ITEM_SIZE; i++)
		fout3 << i + 1 << "   " << test_result_1[i] << "-----" << real_test_result[i] << endl;
	fout3 << flush;
	fout3.close();
