...
for i = 1:101
    for j = 1:24983
        if(data(i,j) == 0)
            item_person(i,j) = average_item(i) + 0.002;
        else
...
%S = load('svd_s.csv');
%V = load('svd_v.csv');
%csvwrite('svd_s.csv', S);
%csvwrite('svd_u.csv', U);
%csvwrite('svd_v.csv', V);
%??S??S??????????????????????????????,????????????????????????????11??????29??
...
for i = 1:101
    for j = 1:24983
        real_final(i,j) = test_ftran(i,j) * sd(j) + average_item(i);
    end
end
...
for i = 1:101
    for j = 1:24983
        %if Index exceed finally, the error is the way to judge whether the
        %item is useful or not
        if(test_data(i,j) == 1 && item_person(i,j) ~= average_item(i) + 0.002)
